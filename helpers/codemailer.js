...
const sendResetCode = async (email, name, code)=> {
    try {
        ...
        const mailOptions = {
          from: EMAIL,
          to: email,
          subject: "Social Media Reset Code",
          text: "Social Media Reset Code",
          html: ` <div style="border: 1px solid #ddd; border-radius: 5px; text-align: center; padding: 20px; background: dimgrey; font-family: 'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande', 'Lucida Sans Unicode', Geneva, Verdana, sans-serif;"> <h1 style="color: azure;">Hey <span style="color: aqua;">${name}</span> You Got An Email</h1> <p style="font-size: 16px; color: beige;">Hello ${name} hope you are well. Please confirm your verification email to join with us.</p> <a style="text-decoration: none;"> <button style="padding: 12px 18px; cursor: pointer; border: none; border-radius: 5px; font-size: 16px; color: #222;" onmouseenter="this.style.background='#2222'" onmouseleave="this.style.background='azure'" >Code => ${code}</button> </a> </div>`
        };
